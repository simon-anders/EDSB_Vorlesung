...
Als erstes klassifiziere ich alle Gene, ob sie sowohl in Herz und Niere, als auch bei der weiblichen und männlichen Gruppe stark exprimiert sind ( >= 1 ).
Diese Tabelle nutze ich später, um die Gene entsprechend einzufärben.
...
humanLongTable %>%
  ...
  mutate(is.low = ifelse(Heart <= 1 & Kidney <= 1, "low", "high")) %>% 
  select(GeneID, Subject, is.low) %>% 
  spread(Subject, is.low) %>% 
  mutate(gene.class = ifelse(Female == "low" & Male1 == "low", "low", "high")) %>% 
